...
from odoo.tools.constants import (
    EXTERNAL_ASSET,
    SCRIPT_EXTENSIONS,
    STYLE_EXTENSIONS,
    SUPPORTED_DEBUGGER,
...
MALICIOUS_SCHEMES = re.compile(
    r"javascript:(?!( ?)((window\.)?)history\.back\(\)$)", re.I
).findall
...
class IrQweb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render_iterall(
        self, view_ref, method, values, directive="render"
    ) -> Iterator[str]:
    ...
    def _get_error_info(
        self, error, stack: list[QwebStackFrame], frame: QwebStackFrame
    ) -> QWebErrorInfo:
    ...
    def _compile(self, template):
        ...
        if isinstance(template, str) and template.endswith(".xml"):
            ...
            if (
                ...
                raise ValueError(
                    "The templates file %s must be under a subfolder 'templates' of a module",
                    template,
                )
            else:
    ...
    def _generate_code(self, template: int | str | etree._Element):
        ...
        if isinstance(template, etree._Element):
            ...
            def_name = TO_VARNAME_REGEXP.sub(
                r"_", f"template_{ref_name if '<' not in ref_name else ''}_{ref}"
            )
    ...
    def _compile_static_node(self, el, compile_context, level):
        ...
        if not el.nsmap:
            ...
            attrib = self._post_processing_att(
                el.tag, {**el.attrib, "__is_static_node": True}
            )
        else:
            # Etree will remove the ns prefixes indirection by inlining the corresponding
            # nsmap definition into the tag attribute. Restore the tag and prefix here.
    ...
    def _compile_directive_att(self, el, compile_context, level):
        ...
        for key in list(el.attrib):
            if key.startswith("t-attf-"):
                ...
                code.append(
                    indent_code(
                        f"attrs[{name!r}] = {self._compile_format(value)}", level
                    )
                )
    ...
    def _compile_directive_call(self, el, compile_context, level):
        ...
        for key in list(el.attrib):
            if key.endswith(".f"):
                ...
                code.append(
                    indent_code(
                        f"t_call_values[{key!r}] = {self._compile_expr(value)}", level
                    )
                )
            elif key == "t-args":
                value = el.attrib.pop(key)
    ...
    def _post_processing_att(self, tagName, atts):
        ...
        """
        if (
            not atts.pop("__is_static_node", False)
            and (href := atts.get("href"))
            and MALICIOUS_SCHEMES(str(href))
        ):
            atts["href"] = ""
        return atts
